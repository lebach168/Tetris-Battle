...
// FindLandingPos return row index after hard drop (space input)
...
func placeBlock(gs *GameState, ps *PlayerState, block [][]int, row, col int) {
	...
	ps.cCol = 4
	ps.blockIndex++
	ps.block = Tetromino[gs.listBlock[ps.blockIndex]]
...
func propagateState(previous *PlayerState, ps *PlayerState) {
	...
	ps.accumulator = previous.accumulator
	ps.block = previous.block
	ps.canHold = previous.canHold
	ps.isCommitting = previous.isCommitting
...
func applyInputBuffer(state *GameState, ps *PlayerState, input InputBuffer) {
	// Order apply: Horizontal move -> Rotate -> Vertical drop -> Hold
